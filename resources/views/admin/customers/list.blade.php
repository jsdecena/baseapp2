                            ...
                            @if($users)
                                <table class="table">
                                    ...
                                    @foreach($users as $user)
                                        <tr>
                                          <td>{{$user->id}}</td>
                                          <td><a href="{{route('admin.customer.edit', $user->id)}}">{{$user->name}}</a></td>
                                          <td>{{$user->email}}</td>
                                          <td>{{$user->created_at->format('d M Y')}}</td>
                                          <td>
                                                {!!Form::open(['url' => route('admin.customer.destroy', $user->id), 'method' => 'delete' ])!!}
                                                    <a href="{{route('admin.customer.edit', $user->id)}}" class="btn btn-sml btn-primary"><i class="fa fa-pencil"></i> Edit</a>
